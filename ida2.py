...
"""
사용 전 확인해야 할 것들
txt파일 저장할 폴더 경로, 크롬 드라이버 있는 경로, 윈도우 맥 tag_.send_keys 변경, 엑셀파일 경로, 엑셀 시트 이름
"""

def create_txt(name, univ, score):
    filename = "/Users/qlover/Desktop/30/" + name + ".txt"  # txt파일 저장 경로 name 앞쪽것만 변경
    f = open(filename, "w")
...
def crawling(name, id, pw):
    try:
        ...
        filename = "/Users/qlover/Desktop/30/" + name + "_more.txt" # txt 파일 저장 경로 name 앞쪽것만 변경
        f = open(filename, "w")
        for i in range(len(univ_list)):
            try:
                driver.find_element_by_xpath('//*[@id="common_Alert_Layer"]/div/div[2]/button').click()
